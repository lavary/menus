...
class Collection implements ArrayAccess, Countable, IteratorAggregate, JsonSerializable
{
    ...
    {
        if (is_null($callback)) {
            return count($this->items) > 0 ? reset($this->items) : Helpers::value($default);
        }
    ...
    {
        if (is_null($callback)) {
            return count($this->items) > 0 ? end($this->items) : Helpers::value($default);
        }
    ...
    {
        $this->each(function ($item) use ($args) {
            $item->setData(func_get_args(), $item->manager->config('cascade_data'));
        });
